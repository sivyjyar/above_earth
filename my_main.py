...
def crop_rect(img, rect, rect1, rect2, rect3, rect4, ix):
    print("rect!")
    ...
    cut_img1 = cv2.getRectSubPix(img_rot, size, center1)
    cut_img2 = cv2.getRectSubPix(img_rot, size, center2)
    cut_img3 = cv2.getRectSubPix(img_rot, size, center3)
    cut_img4 = cv2.getRectSubPix(img_rot, size, center4)
    # сохранить часть изображения
    cv2.imwrite(f'parts0/{ix}_part.jpg', cut_img)


    cv2.imwrite(f'parts1/{ix}_part.jpg', cut_img1)
    cv2.imwrite(f'parts2/{ix}_part.jpg', cut_img2)
    cv2.imwrite(f'parts3/{ix}_part.jpg', cut_img3)
    cv2.imwrite(f'parts4/{ix}_part.jpg', cut_img4)
...
#                rect = ((a[0], a[1]), (100, 200), angle_)
                ...
                rect_2 = (float(a[0]+5), float(a[1]))
                rect_3 = (float(a[0]+10), float(a[1]))
                rect_4 = (float(a[0]+15), float(a[1]))
                rect_5 = (float(a[0]+20), float(a[1]))
                # вырезать кусок картинки из общего изображения
                img_array2_ = crop_rect(img_array, rect_, rect_2, rect_3, rect_4, rect_5, ix)
