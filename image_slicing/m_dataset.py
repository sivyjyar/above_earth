...
def crop_rect(img, rect, ix, n_n):
    ...
    ix1 = ix + 33*(n_n-1)
    cv2.imwrite(f'data_set_5km/full_set/{ix1}.jpg', cut_img)
...
#                rect = ((a[0], a[1]), (100, 200), angle_)
                ...
                img_array2_ = crop_rect(img_array, rect_, ix, n_n)
...
#                          pts[ix][0]-(height_window//2):pts[ix][0]+(height_window//2), :] = 255    
    ...
    def loadImage(self, n_n):
        # загрузить изображение
        self.img = Image.open('data_set_5km/'+str(n_n)+'.png')
        size = self.img.size
